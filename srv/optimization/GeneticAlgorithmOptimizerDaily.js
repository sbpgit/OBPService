...
class GeneticAlgorithmOptimizerDaily {
  constructor(planningSystem, options = {}) {
    ...
    this.promiseDatePreference = options.promiseDatePreference || 0.7;
    this.timingVarianceDays = options.timingVarianceDays || 7;
    this.unnecessaryDelayPenalty = options.unnecessaryDelayPenalty || 100;
    this.perfectTimingBonus = options.perfectTimingBonus || 50;
    ...
    // FIXED: Add strict capacity checking flag
    this.strictCapacityCheck = options.strictCapacityCheck !== false;
  }
  ...
  createIndividual() {
    ...
    // FIXED: Create proper capacity tracking that mirrors actual system capacity
    const capacityTracker = this.initializeStrictCapacityTracker();
    ...
    for (const orderNumber of sortedOrders) {
      ...
      // FIXED: Use strict capacity checking
      const bestDay = this.findBestAvailableDayStrict(order, targetDayIndex, capacityTracker);
      ...
      if (bestDay.dayIndex >= 0) {
        ...
        // FIXED: Update capacity tracker to reflect actual usage
        this.updateStrictCapacityTracker(capacityTracker, order, bestDay.dayIndex, bestDay.operationsAssignment);
      } else {
        // FIXED: If no capacity available, assign to a late date but mark as invalid
        const lateDayIndex = Math.min(targetDayIndex + 30, this.system.dailyBuckets.length - 1);
        individual[orderNumber] = {
          dayIndex: lateDayIndex,
          operationsAssignment: this.assignOperations(orderNumber),
          invalidAssignment: true // Mark as invalid due to no capacity
        };
      }
    }
  ...
  // FIXED: Initialize capacity tracker with exact daily capacity values
  initializeStrictCapacityTracker() {
    const tracker = {};

    for (const [lineName, restriction] of this.system.lineRestrictions.entries()) {
      if (restriction && restriction.validity !== false && restriction.dailyCapacity) {
        tracker[lineName] = {};
        
        for (let dayIndex = 0; dayIndex < this.system.dailyBuckets.length; dayIndex++) {
          const day = this.system.dailyBuckets[dayIndex];
          const capacity = restriction.dailyCapacity[day.date] || 0;
          tracker[lineName][dayIndex] = capacity;
        }
      }
    }
  ...
  // FIXED: Strict capacity search that respects zero-capacity days
  findBestAvailableDayStrict(order, targetDayIndex, capacityTracker) {
    const orderEarliestDay = this.system.getEarliestSchedulableDayForOrder(order.orderNumber);
    const priorityCriteria = this.system.getPriorityDeliveryCriteria(order.customerPriority);
    const maxDelayDays = priorityCriteria.maxDelayDays;
    ...
    const searchStart = orderEarliestDay;
    const searchEnd = Math.min(
      targetDayIndex + maxDelayDays + 14, // Extended search window
      this.system.dailyBuckets.length - 1
    );

    // Build search order: target first, then spiral outward
    const searchOrder = [];
    if (targetDayIndex >= searchStart && targetDayIndex <= searchEnd) {
      searchOrder.push(targetDayIndex);
    }
    ...
    // FIXED: Try each day in order, checking actual capacity
    for (const dayIndex of searchOrder) {
      const assignment = this.findStrictCapacityForDay(order, dayIndex, capacityTracker);
      if (assignment) {
        return {
          dayIndex: dayIndex,
          operationsAssignment: assignment
  ...
  // FIXED: Strict capacity checking for a specific day
  findStrictCapacityForDay(order, dayIndex, capacityTracker) {
    if (dayIndex < 0 || dayIndex >= this.system.dailyBuckets.length) {
      return null;
    }

    const assignment = {};
    const tempReservations = {}; // Track what we're planning to reserve
    ...
    for (const operationId of order.operations) {
      ...
      for (const lineName of availableLines) {
        ...
        const currentAvailableCapacity = capacityTracker[lineName][dayIndex];
        const alreadyReserved = tempReservations[lineName] || 0;
        const remainingCapacity = currentAvailableCapacity - alreadyReserved;

        // STRICT CHECK: Must have exact capacity available
        if (remainingCapacity >= order.orderQty) {
          assignment[operationId] = lineName;
          tempReservations[lineName] = alreadyReserved + order.orderQty;
          lineFound = true;
          break;
        }
      }
      ...
      if (!lineFound) {
        return null; // Cannot fit all operations, return null
      }
    }
  ...
  // FIXED: Update capacity tracker by reducing available capacity
  updateStrictCapacityTracker(capacityTracker, order, dayIndex, operationsAssignment) {
    for (const [operationId, lineName] of Object.entries(operationsAssignment)) {
      if (capacityTracker[lineName] && capacityTracker[lineName][dayIndex] !== undefined) {
        capacityTracker[lineName][dayIndex] -= order.orderQty;
        // Ensure capacity never goes negative
        capacityTracker[lineName][dayIndex] = Math.max(0, capacityTracker[lineName][dayIndex]);
      }
    }
  }
  ...
  calculateFitness(individual) {
    let totalPenalty = 0.0;
    const capacityUsageTracker = {}; // Track actual usage for penalty calculation
    const earliestDay = this.system.getEarliestSchedulableDayIndex();

    // Initialize capacity usage tracking
    for (const [lineName] of this.system.lineRestrictions) {
      capacityUsageTracker[lineName] = {};
    }
    ...
    for (const [orderNumber, assignment] of Object.entries(individual)) {
      ...
      // FIXED: Severe penalty for invalid assignments
      if (assignment.invalidAssignment) {
        totalPenalty += 200000; // Massive penalty for invalid assignments
        invalidAssignments++;
        continue;
      }
      ...
      if (daysDifference > 0) {
        ...
        const penaltyRule = this.system.penaltyRules.get(penaltyKey);
        if (penaltyRule) {
          const daysDivided = Math.max(1, Math.floor(daysDifference / 7));
          const latePenalty = penaltyRule.lateDeliveryPenalty * Math.pow(daysDivided + 1, 1.5) * priorityCriteria.penaltyMultiplier;
          totalPenalty += latePenalty;
        }
      } else if (daysDifference === 0) {
      ...
      // FIXED: Strict capacity constraint checking
      const scheduledDateStr = scheduledDate.format('YYYY-MM-DD');
      for (const [operationId, lineRestriction] of Object.entries(assignment.operationsAssignment)) {
        if (!capacityUsageTracker[lineRestriction]) {
          capacityUsageTracker[lineRestriction] = {};
        }
        if (!capacityUsageTracker[lineRestriction][dayIndex]) {
          capacityUsageTracker[lineRestriction][dayIndex] = 0;
        }

        capacityUsageTracker[lineRestriction][dayIndex] += order.orderQty;

        const restriction = this.system.lineRestrictions.get(lineRestriction);
        if (restriction && restriction.dailyCapacity) {
          const maxCapacity = restriction.dailyCapacity[scheduledDateStr] || 0;
          const currentUsage = capacityUsageTracker[lineRestriction][dayIndex];

          // FIXED: Severe penalty for violating capacity, especially zero-capacity days
          if (currentUsage > maxCapacity) {
            const excess = currentUsage - maxCapacity;
            
            // Extra severe penalty if trying to schedule on zero-capacity day
            if (maxCapacity === 0) {
              totalPenalty += excess * 10000; // Massive penalty for zero-capacity violations
              severeCapacityViolations += excess;
            } else {
              const capacityPenalty = restriction.penaltyCost * Math.pow(excess, 2); // Quadratic penalty
              totalPenalty += capacityPenalty;
            }

            totalCapacityViolations += excess;

            if (excess >= maxCapacity && maxCapacity > 0) {
              severeCapacityViolations++;
              totalPenalty += 5000;
            }
          }
      ...
      for (const [component, requiredQty] of Object.entries(order.components)) {
        const availability = this.system.componentAvailability.get(component);
        if (availability && availability.dailyAvailability) {
          const available = availability.dailyAvailability[scheduledDateStr] || 0;
          if (requiredQty > available) {
            totalPenalty += (requiredQty - available) * 50;
          }
    ...
    // Additional penalties for poor patterns
    if (severeCapacityViolations > 0) {
      totalPenalty += severeCapacityViolations * 2000;
    }
    ...
    if (invalidAssignments > 0) {
      totalPenalty += invalidAssignments * 50000; // Heavy penalty for invalid assignments
    }

    // Under-utilization penalties (if enabled)
    if (this.enableUnderUtilizationPenalty) {
      const underUtilizationPenalty = this.calculateUnderUtilizationPenalties(capacityUsageTracker);
      totalPenalty += underUtilizationPenalty * this.underUtilizationWeight;
    }
  ...
  mutate(individual) {
    const earliestDay = this.system.getEarliestSchedulableDayIndex();
    const capacityTracker = this.calculateCurrentStrictCapacityUsage(individual);
    ...
    for (const orderNumber of Object.keys(individual)) {
      if (Math.random() < this.mutationRate) {
        ...
        this.releaseStrictCapacityTracker(capacityTracker, order, currentAssignment.dayIndex, currentAssignment.operationsAssignment);
        ...
        // 80% chance for intelligent mutation (capacity-aware), 20% random
        if (Math.random() < 0.8) {
          const betterDay = this.findBestAvailableDayStrict(order, targetDayIndex, capacityTracker);
          ...
          if (betterDay.dayIndex >= 0) {
            ...
            this.updateStrictCapacityTracker(capacityTracker, order, betterDay.dayIndex, betterDay.operationsAssignment);
          } else {
            // Keep current assignment if no better option found
            individual[orderNumber] = currentAssignment;
            this.updateStrictCapacityTracker(capacityTracker, order, currentAssignment.dayIndex, currentAssignment.operationsAssignment);
          }
        } else {
          // Random mutation within reasonable bounds
          const maxDay = Math.min(earliestDay + 140, this.system.dailyBuckets.length - 1);
          let attempts = 0;
          let newDayIndex;
          let newAssignment;
          ...
          do {
            newDayIndex = Math.floor(Math.random() * (maxDay - orderEarliestDay + 1)) + orderEarliestDay;
            newAssignment = this.findStrictCapacityForDay(order, newDayIndex, capacityTracker);
            attempts++;
          } while (!newAssignment && attempts < 10);
          ...
          if (newAssignment) {
            ...
            this.updateStrictCapacityTracker(capacityTracker, order, newDayIndex, newAssignment);
          } else {
            individual[orderNumber] = currentAssignment;
            this.updateStrictCapacityTracker(capacityTracker, order, currentAssignment.dayIndex, currentAssignment.operationsAssignment);
          }
  ...
  calculateCurrentStrictCapacityUsage(individual) {
    const capacityTracker = this.initializeStrictCapacityTracker();

    for (const [orderNumber, assignment] of Object.entries(individual)) {
      const order = this.system.salesOrders.get(orderNumber);
      if (order && !assignment.invalidAssignment) {
        this.updateStrictCapacityTracker(capacityTracker, order, assignment.dayIndex, assignment.operationsAssignment);
      }
    }
  ...
  releaseStrictCapacityTracker(capacityTracker, order, dayIndex, operationsAssignment) {
    for (const [operationId, lineName] of Object.entries(operationsAssignment)) {
      if (capacityTracker[lineName] && capacityTracker[lineName][dayIndex] !== undefined) {
        capacityTracker[lineName][dayIndex] += order.orderQty;
      }
  ...
  async optimize() {
    console.log('Starting genetic algorithm optimization with STRICT daily capacity validation...');
